...
module.exports = function sendEmail(options, callback) {
  ...
  var emailsTo = [];
  var emailList = options.payload.to;

  if (typeof emailList === 'string') {
    emailsTo.push(emailList);
  }else {
    emailsTo = emailList;
  }
  ...
  emailsTo.forEach(function (e) {
    console.log('Emails inside ', e);
  ...
  // see: http://stackoverflow.com/questions/30590988/failed-sending-mail-through-google-api-with-javascript
      params = { userId: 'me', auth: oauth2Client, resource: { //  mested object
